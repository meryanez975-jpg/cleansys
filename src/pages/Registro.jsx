...
// ── Cronómetro ────────────────────────────────────────────────────
// Persiste entre cierres de la app porque lee hora_entrada desde localStorage
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
        <button
          ...
          style={{ background: 'var(--primary-light)', border: 'none', borderRadius: 8, padding: '5px 12px', cursor: 'pointer', color: 'var(--primary-dark)', fontWeight: 700, fontSize: 16 }}
        >‹</button>
        <div style={{ textAlign: 'center' }}>
          <p style={{ fontWeight: 700, fontSize: 14, color: 'var(--text)', textTransform: 'capitalize' }}>
            {formatMesLargo(mes)}
          </p>
          <p style={{ fontSize: 11, color: '#ef4444', fontWeight: 600 }}>
            {cantAsig} limpieza{cantAsig !== 1 ? 's' : ''} asignada{cantAsig !== 1 ? 's' : ''}
          </p>
        </div>
        <button
          onClick={() => setMes(m => addMes(m, 1))}
          ...
          return (
            <div key={i} style={{
              ...
              <span style={{
                ...
                color: esHoy ? 'var(--primary-dark)' : 'var(--text)',
              }}>
                {d}
              </span>
              {/* Puntito rojo si tiene asignación */}
              <span style={{
                width: 6, height: 6, borderRadius: '50%',
                background: tieneAsig ? '#ef4444' : 'transparent',
                flexShrink: 0,
              }} />
            </div>
          )
    ...
    <div className="page">
      <div className="container">
        ...
        <div style={{ marginBottom: 16 }}>
          ...
          >
            <span style={{ fontWeight: 700, fontSize: 14, color: showCal ? '#fff' : 'var(--text)' }}>
              📅 Mis limpiezas del mes
            </span>
            <span style={{ color: showCal ? '#fff' : 'var(--text-muted)', fontSize: 13 }}>
              {showCal ? '▲' : '▼'}
            </span>
          </button>
